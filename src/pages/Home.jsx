...
const Home = () => {
  ...
  <div className="absolute inset-0 flex items-center justify-end px-6 md:px-24">
    <div className="max-w-lg flex flex-col items-center text-center">
      ...
      <p className="font-inter font-normal text-[16px] leading-[24px] text-[#535353] mb-8 max-w-sm">
        {hero.subtitle}
      </p>
      <button className="bg-[#d4a44f] text-black px-10 py-3 rounded-md font-bold hover:bg-[#b98d3f] transition duration-300 uppercase tracking-wider text-sm">
        {hero.button}
      </button>
      ...
      <section className="relative bg-orange-50 py-16 px-6 md:px-12">
        <div className="flex justify-between items-center mb-12">
          <h2 className="text-4xl font-serif text-[#2f2f2f]">{collections.heading}</h2>
          <button className="text-sm underline text-[#2f2f2f]">{collections.button}</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {collections.items.slice(0, 2).map((item) => (
            <div key={item.id}>
              ...
              />
              <h3 className="mt-5 text-xl font-medium text-[#2f2f2f]">{item.title}</h3>
            </div>
          ))}
      ...
      <section className="relative bg-[rgba(250,238,227,1)] py-16 px-6 md:px-12 overflow-hidden">
        <div className="flex justify-between items-center mb-12">
          <h2 className="text-3xl font-serif text-[#2f2f2f]">{bestSellers.heading}</h2>
          <button className="text-md underline text-[#2f2f2f]">{bestSellers.button}</button>
        </div>
  ...
  <div className="max-w-8xl mx-auto">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      {homeData.whySection.items.map((item) => (
        <div 
          key={item.id} 
          className="bg-[#e8e0d6] rounded-xl p-17 text-center flex flex-col items-center"
        >
      ...
      <section className="w-full relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-end justify-center text-center px-6 pb-10 md:pb-20">
          <div className="max-w-lg">
            <h1 className="text-2xl md:text-5xl text-white mb-6 font-javanese">{banner.title}</h1>
            <button className="bg-white text-black px-6 md:px-8 py-2 md:py-3 rounded-md font-semibold hover:bg-[#ddd7cd] transition duration-300">
              {banner.button}
            </button>
          </div>
    ...
    <section className="w-full bg-white py-10">
      <div className="max-w-[1400px] mx-auto flex flex-col gap-5">
        {categories.map((cat) => (
          <div 
            key={cat.id} 
            className={`flex flex-col md:flex-row ${cat.reverse ? 'md:flex-row-reverse' : ''} items-stretch w-full `}
          >
            {/* Image Box */}
            <div className="w-full md:w-1/2">
              <img 
                src={cat.image} 
                alt={cat.title} 
                className="w-[673px] h-[538px] rounded-lg object-cover" 
              />
            </div>
            ...
            <div className="w-full md:w-1/2 flex flex-col items-center justify-center p-10 md:p-24 text-center relative bg-white">
              ...
              {/* Decorative Flower - Absolute Positioned */}
              {/* <img 
                src={flowerIcon} 
                className={`absolute w-40 md:w-64 opacity-20 pointer-events-none 
                  ${cat.reverse ? 'bottom-0 left-0 -translate-x-10' : 'top-0 right-0 translate-x-10'}`} 
                alt="flower" 
              /> */}
              ...
              <div className="relative z-10 max-w-md">
                ...
                <button className="bg-[#1a1a1a] text-white rounded-md px-8 py-3 text-md md:text-sm font-semibold tracking-widest hover:bg-gray-800 transition-colors uppercase">
                  SHOP NOW
                </button>
              </div>
            </div>
      ...
      <section className="bg-[rgba(250,238,227,1)] py-10 px-6 relative">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-20 items-start">
          <div className="space-y-5 mt-40 md:col-span-1">
            <h2 className="text-3xl font-serif text-gray-600">{reviews.heading}</h2>
            <p className="text-gray-600 text-md leading-6">{reviews.subtext}</p>
            <a href="#" className="underline text-gray-800 text-xl font-medium">{reviews.button}</a>
          </div>

          <div className="md:col-span-3 grid sm:grid-cols-2 md:grid-cols-3 gap-10">
            {reviews.items.map((item) => (
              <div key={item.id} className="space-y-4">
                <img src={item.image} alt="review" className="w-full h-[290px] object-cover rounded-xl" />
                <div className="flex gap-6 text-yellow-400 text-3xl">★ ★ ★ ★ ★</div>
                <p className="font-inter font-normal not-italic text-[17px] leading-[24px] tracking-normal text-[#535353]">{item.text}</p>
                <p className="text-[#535353] text-base font-normal font-inter">{item.name}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 text-center">
          {features.map((f) => (
            <div key={f.id} className="space-y-3">
              <img src={f.image} alt={f.title} className="w-16 h-16 mx-auto" />
      ...
      <section className="bg-[rgba(250,238,227,1)] py-24">
        <div className="text-center">
          <div className="flex items-center justify-center gap-6 mb-6 px-4">
            ...
            <h2 className="font-serif text-xl md:text-xl text-[#2f2f2f]">{instagram.heading}</h2>
            <div className="h-[1px] bg-[#9c8f82] w-36 md:w-150"></div>
          </div>
          <p className="text-[#5e5e5e] mb-12 px-4">{instagram.subtext}</p>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
            {instagram.images.map((img, i) => (
              <img key={i} src={img} className="w-full rounded-xl aspect-[3/4] object-cover" />
            ))}
          </div>
